...
#####################################################################
data_dir_prefix = "/charybdis/toomre/ryor5023/Projects/Rayleigh-Taylor/RT/"
#####################################################################
...
# parse command args
try:
    opts, args = getopt.getopt(sys.argv[1:], "", ["Re=", "Pr=", "At=", 
            "Lx=", "Lz=", "tstop=", "twall-stop=", "nx=", "nz=", "dir-str="])
...
pde = RT.set_problem(Reynolds, Prandtl, Atwood)

if domain.distributor.rank == 0:
  if not os.path.exists('{:s}/'.format(data_dir)):
    os.mkdir('{:s}/'.format(data_dir))

# output starting values of various parameters
...
if Noise_IC:
    ...
    if (stable):
        ...
        #w['g'] = A_u*np.cos(2*np.pi*x/Lx)
        w['g'] = -A_u*np.exp(-(x-0.5*Lx)**2/(width**2))
